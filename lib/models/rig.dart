...
import '../services/auto_rig_cutout_service.dart';
...
class SceneActor {
  ...
  void captureFrom(ProjectState p) {
    ...
    parts = [for (final pt in p.parts) CharacterPart(id: pt.id, name: pt.name, type: pt.type, boneId: pt.boneId, visible: pt.visible, locked: pt.locked, x: pt.x, y: pt.y, rotation: pt.rotation, scale: pt.scale, crop: pt.crop, imagePath: pt.imagePath, imageOffsetX: pt.imageOffsetX, imageOffsetY: pt.imageOffsetY)];
    mesh = p.mesh.copy();
    animationLayers
      ..clear()
...
class CharacterPart {
  CharacterPart({required this.id, required this.name, required this.type, required this.boneId, this.visible = true, this.locked = false, this.x = 0, this.y = 0, this.rotation = 0, this.scale = 1, this.crop, this.imagePath = '', this.imageOffsetX = 0, this.imageOffsetY = 0});
  final String id;
  final String name;
  final PartType type;
  String boneId;
  ...
  Rect? crop;
  /// Optional persistent transparent cut-out generated by Auto-Rig.
  /// Empty means the original imported image + crop is used.
  String imagePath;
  /// Source-art anchor correction in scene units. The Auto-Rig computes this
  /// so the anatomical joint (shoulder/elbow/hip...) lands on the matching bone.
  double imageOffsetX;
  double imageOffsetY;
}
...
class ProjectState extends ChangeNotifier {
  ...
  /// Dedicated raster exposure track for true frame-by-frame animation.
  /// It coexists with the procedural/rig animation system instead of replacing it.
  final FrameAnimationTrack frameAnimation = FrameAnimationTrack();
  int frameCursor = 0;
  /// When true, the viewport shows the hand-drawn exposure sheet instead of
  /// the rig — a real on/off switch, not an implicit "if there happens to
  /// be a drawing" guess, because an implicit toggle is exactly the kind of
  /// thing that silently does nothing and looks like the feature is broken.
  bool frameEditorEnabled = false;
  void setFrameEditorEnabled(bool v) { frameEditorEnabled = v; status = v ? 'Preview: hand-drawn frames' : 'Preview: rig'; notifyListeners(); }
  /// Which exposure is on screen right now, driven by the same playhead
  /// that drives rig playback — so Play/the timeline scrubber moves the
  /// drawing too, instead of frame-by-frame content being frozen unless you
  /// go back into its own editor and click through frames by hand.
  int get frameAnimationIndexAtPlayhead {
    if (frameAnimation.length == 0) return 0;
    return (playhead * frameAnimation.fps).floor().clamp(0, frameAnimation.length - 1);
  }

  int get currentFrame => frameCursor.clamp(0, math.max(0, frameAnimation.length - 1)).toInt();
  void setFrameCursor(int index) {
    frameCursor = index.clamp(0, math.max(0, frameAnimation.length - 1));
    setPlayhead(frameAnimation.length == 0 ? 0 : frameCursor / frameAnimation.fps);
    notifyListeners();
  }
  void frameStep(int delta) => setFrameCursor(frameCursor + delta);
  void ensureFrameTrack() { frameAnimation.ensureFrame(frameCursor); notifyListeners(); }
  ...
  void toggleUseImageAsBody(bool value) { useImageAsBody = value; persistSelectedActor(); status = value ? 'Using uploaded image as body' : 'Using drawn character'; notifyListeners(); }
  ...
  Future<void> autoMapImageParts() async {
    if (importedImagePath.isEmpty) {
      status = 'Importe une image avant Auto-Rig';
      notifyListeners();
      return;
    }
    status = 'Auto-Rig: analyse du personnage…';
    notifyListeners();
    var headSplit = .18;
    var legSplit = .52;
    ...
    status = 'Auto-Rig: segmentation pixel par pixel…';
    notifyListeners();
    try {
      final build = await AutoRigCutoutService.build(importedImagePath);
      for (final part in parts) {
        part.imagePath = build.cutouts[part.id] ?? '';
        final crop = build.crops[part.id];
        if (crop != null && crop.length == 4) {
          part.crop = Rect.fromLTWH(crop[0], crop[1], crop[2], crop[3]);
        }
        final offset = build.offsets[part.id];
        if (offset != null && offset.length == 2) {
          part.imageOffsetX = offset[0];
          part.imageOffsetY = offset[1];
        }
      }
      useImageAsBody = true;
      // The imported artwork belongs to the selected actor, not the procedural demo character.
      persistSelectedActor();
      status = 'Auto-Rig terminé — personnage importé prêt à être posé';
      notifyListeners();
    } catch (e) {
      status = 'Auto-Rig partiel — régions créées, textures séparées indisponibles: $e';
      persistSelectedActor();
      notifyListeners();
    }
  }
